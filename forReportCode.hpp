...
class HSICamera
{
    ...
    private:
      ...
      const int nRawImagesInMemory = 25;
    ...
    for(int row=0; row<sensorRows; row++){
      ...
      for(int binnIterator=0; binnIterator<nFullBinnsPerRow; binnIterator++){
        ...
        */
        ...
        insertionSort(rawImageP, binningFactor);
      }
...
void HSICamera::swap(char *xp, char *yp)
{
    char temp = *xp;
    *xp = *yp;
    *yp = temp;
...
void HSICamera::insertionSort(char arr[], int n)
{
  char key;
   int i, j;
   for (i = 1; i < n; i++)
...
void HSICamera::bubbleSort(char arr[], int n)
{
   int i, j;
   for (i = 0; i < n-1; i++)
