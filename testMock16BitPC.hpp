...
void HSICamera::freeRunCapture(){
  ...
  for(int imageNumber=0; imageNumber<nSingleFrames; imageNumber++){
    ...
    size_t result = fread (pointerToNew16BitArray, 2, sensorRows*sensorColumns, pFile);

    if (result != sensorRows*sensorColumns) {fputs ("Reading error",stderr); exit (3);}
    ...
    for(int row=0; row<sensorRows; row++){
      ...
      for(int binnIterator=0; binnIterator<nFullBinnsPerRow; binnIterator++){
        ...
        bubbleSort(pointerToNew16BitArray+rowAndBinOffset, 12);
  ...
  if(cubeType==Bil){//BIL
    ...
    for(int band=0; band<nFullBinnsPerRow; band++){
      for(int rowSpatial=0; rowSpatial<nSingleFrames; rowSpatial++){
        ...
        fwrite (cubeColumn, sizeof(uint16_t), cubeColumns, pFileCube);
      }
    }
    fclose (pFileCube);
  }
