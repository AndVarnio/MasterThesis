...
/////////////////Freerun mode //////////////
  ...
  if(1){//if this is bsq
    ...
    for(int imageNumber=0; imageNumber<nSingleFrames; imageNumber++){
      printf("Reading Image nr: %i\n", imageNumber);
      is_WaitForNextImage(hCam, 1000, &ppcMem[imageNumber], &imageSequenceID);
      // is_LockSeqBuf (hCam, IS_IGNORE_PARAMETER, ppcMem[imageNumber]);
      printf("Image nr read: %i\n", imageNumber);
      /////Binning
      sleep(2);
      if(binningFactor>1){
        if(meanBinning){
        ...
        int nColumnsBinned = (bands + binningFactor - 1) / binningFactor;
          ...
          for(int row=0; row<sensorRows; row++){
            for(int binnIterator=0; binnIterator<nBinningsPerRow-1; binnIterator++){
              int totPixVal = 0;
              for(int pixelIterator=0; pixelIterator<binningFactor-1; pixelIterator++){
                // TODO Char arithmatic

                totPixVal += (int)ppcMem[imageNumber][row*sensorColumns+binnIterator*binningFactor+pixelIterator];
                // totPixVal += (int)ppcMem[imageNumber][1];
              }
              binnedImages[imageNumber][row*nColumnsBinned+binnIterator] = (unsigned char)(totPixVal/binningFactor);
            ...
            if(factorLastBands>0){
              ...
              binnedImages[imageNumber][row*nColumnsBinned+nBinningsPerRow] = (unsigned char)totPixVal/factorLastBands;
            }
          }
        }
        else{
      ...
      // is_UnlockSeqBuf (hCam, IS_IGNORE_PARAMETER, ppcMem[imageNumber]);
      // printf("Tick %i\n", imageNumber);
    }
